...
import boom_v1.regfile.{MStatus, PTBR}

import scala.collection.mutable.ListBuffer

class PTWReq(implicit p: Parameters) extends Bundle()(p) {
  val prv = Bits(width = 2)
  val pum = Bool()
  val mxr = Bool()
  val addr = UInt(width = p.vpnBits)
  val store = Bool()
  val fetch = Bool()
}

class PTWResp(implicit p: Parameters) extends Bundle()(p) {
  val pte = new PTE
}

class TLBPTWIO(implicit p: Parameters) extends Bundle()(p) {
  val req = master Stream(new PTWReq)
...
class DatapathPTWIO(implicit p: Parameters) extends Bundle()(p) {
  val ptbr = new PTBR().asInput
  val invalidate = Bool(INPUT)
  val status = new MStatus().asInput
}

class PTE(implicit p: Parameters) extends Bundle()(p) {
...
class PTW(n: Int)(implicit p: Parameters) extends Module()(p) {
  val io = new Bundle {
