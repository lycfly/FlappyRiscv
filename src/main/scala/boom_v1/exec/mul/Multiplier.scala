...
class MultiplierIO(dataBits: Int, tagBits: Int) extends Bundle {
  val req = slave Stream(new MultiplierReq(dataBits, tagBits))
  val kill = Bool(INPUT)
  val resp = master Stream(new MultiplierResp(dataBits, tagBits))
}
...
//  val s_ready :: s_neg_inputs :: s_busy :: s_move_rem :: s_neg_output :: s_done :: Nil = Enum(UInt(), 6)
  ...
  val cmdMul :: cmdHi :: lhsSigned :: rhsSigned :: Nil =
    DecodeLogic(io.req.payload.fn.asUInt, List(X, X, X, X), List(
                   FN_DIV    -> List(N, N, Y, Y),
                   FN_REM    -> List(N, Y, Y, Y),
