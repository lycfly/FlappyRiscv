...
import boom_v1.{CSRs, Causes, MaskedDC, Parameters}
...
class MStatus extends Bundle {
  ...
  val debug = Bool()
  val isa = UInt(width = 32 bits)

  val prv = UInt(width = PRV.SZ bits) // not truly part of mstatus, but convenient
  val sd = Bool()
  val zero3 = UInt(width = 31 bits)
  val sd_rv32 = Bool()
  val zero2 = UInt(width = 2 bits)
  val vm = UInt(width = 5 bits)
  val zero1 = UInt(width = 4 bits)
  val mxr = Bool()
  val pum = Bool()
  val mprv = Bool()
  val xs = UInt(width = 2 bits)
  val fs = UInt(width = 2 bits)
  val mpp = UInt(width = 2 bits)
  val hpp = UInt(width = 2 bits)
  val spp = UInt(width = 1 bits)
  val mpie = Bool()
  val hpie = Bool()
  val spie = Bool()
  val upie = Bool()
...
class DCSR extends Bundle {
  val xdebugver = UInt(width = 2 bits)
  val ndreset = Bool()
  val fullreset = Bool()
  val zero3 = UInt(width = 12 bits)
  val ebreakm = Bool()
  val ebreakh = Bool()
  val ebreaks = Bool()
  val ebreaku = Bool()
  ...
  val stopcycle = Bool()
  val stoptime = Bool()
  val cause = UInt(width = 3 bits)
  val debugint = Bool()
  val zero1 = Bool()
  val halt = Bool()
  val step = Bool()
  val prv = UInt(width = PRV.SZ bits)
}
...
class PTBR(implicit p: Parameters) extends Bundle {
  require(p.maxPAddrBits - p.pgIdxBits + p.ASIdBits <= p.xLen)
  val asid = UInt(width = p.ASIdBits bits)
  val ppn = UInt(width = p.maxPAddrBits - p.pgIdxBits bits)
}
