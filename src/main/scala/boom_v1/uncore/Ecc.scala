...
class ParityCode extends Code
{
  ...
  def encode(x: UInt) = Cat(x.xorR, x).asUInt
  def decode(y: UInt) = new Decoding {
    def uncorrected = y(y.getWidth-2 downto 0)
...
class SECCode extends Code
{
  ...
  def decode(y: UInt) = new Decoding {
    ...
    private def swizzle(z: UInt) = (1 to n).filter(i => !isPow2(i)).map(i => z(i-1)).asBits().asUInt
    def uncorrected = swizzle(y)
    def corrected = swizzle(((y << 1) ^ UIntToOh(syndrome).asUInt) >> 1)
...
class SECDEDCode extends Code
{
  ...
  def decode(x: UInt) = new Decoding {
    val secdec = sec.decode(x(x.getWidth-2 downto 0))
...
object ErrGen
{
  ...
  def apply(width: Int, f: Int): UInt = {
    ...
    UIntToOh(LFSR16()(log2Up(width)+f-1 downto 0))(width-1 downto 0).asUInt
  }
