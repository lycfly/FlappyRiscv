...
class RobPCRequest(implicit p: Parameters) extends Bundle
{
  val rob_idx  = in UInt(p.ROB_ADDR_SZ bits)
  val curr_pc  = out UInt(p.vaddrBits+1 bits)
  val curr_brob_idx = out UInt(p.BROB_ADDR_SZ bits)
  // the next_pc may not be valid (stalled or still being fetched)
  val next_val = out Bool()
  val next_pc  = in UInt(p.vaddrBits+1 bits)
}


class DebugRobSignals(implicit p: Parameters) extends Bundle
{
  val state = UInt()
  val rob_head = UInt(width = p.ROB_ADDR_SZ bits)
  val xcpt_val = Bool()
  val xcpt_uop = new MicroOp()
  val xcpt_badvaddr = UInt(width = p.xLen bits)
}
