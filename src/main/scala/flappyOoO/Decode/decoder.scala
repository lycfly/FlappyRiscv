...
case class uop_handle(
                       val rs1: rg_handle = rg_handle(), val rs2: rg_handle= rg_handle(), val rd: rg_handle= rg_handle(),
                       val cycles: Int = 1, val fuType: SpinalEnumElement[FU_TYPE.type] = FU_TYPE.ALU
                     ) {
}
...
case class decoder()(implicit conf: Config) extends Component {
  ...
  def addOPs_RvI(config: Config): Unit = {
    ...
    // load/store
    addDecoding(Opcodes.LW, InstructionType.I, mutable.Map(UOPs.LOAD -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM), cycles = conf.Loader.cycles_fixed, fuType = FU_TYPE.LSU)))
    addDecoding(Opcodes.LH, InstructionType.I, mutable.Map(UOPs.LOAD -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM), rd = rg_handle(width = AccessWidth.H), cycles = conf.Loader.cycles_fixed, fuType = FU_TYPE.LSU)))
    addDecoding(Opcodes.LB, InstructionType.I, mutable.Map(UOPs.LOAD -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM), rd = rg_handle(width = AccessWidth.B), cycles = conf.Loader.cycles_fixed, fuType = FU_TYPE.LSU)))
    addDecoding(Opcodes.LHU, InstructionType.I, mutable.Map(UOPs.LOADU -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM), rd = rg_handle(width = AccessWidth.H), cycles = conf.Loader.cycles_fixed, fuType = FU_TYPE.LSU)))
    addDecoding(Opcodes.LBU, InstructionType.I, mutable.Map(UOPs.LOADU -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM), rd = rg_handle(width = AccessWidth.B), cycles = conf.Loader.cycles_fixed, fuType = FU_TYPE.LSU)))
    addDecoding(Opcodes.SW, InstructionType.S, mutable.Map(UOPs.STORE -> uop_handle(fuType = FU_TYPE.LSU)))
    ...
    addDecoding(Opcodes.BLT, InstructionType.B, mutable.Map(UOPs.BLT -> uop_handle(fuType = FU_TYPE.BRU)))
    addDecoding(Opcodes.BGE, InstructionType.B, mutable.Map(UOPs.BGE -> uop_handle(fuType = FU_TYPE.BRU)))
    addDecoding(Opcodes.BLTU, InstructionType.B, mutable.Map(UOPs.BLTU -> uop_handle(fuType = FU_TYPE.BRU)))
    addDecoding(Opcodes.BGEU, InstructionType.B, mutable.Map(UOPs.BGEU -> uop_handle(fuType = FU_TYPE.BRU)))
  ...
  def addOps_RvZicsr(config: Config): Unit = {
    if(config.RvZicsr){
      addDecoding(Opcodes.CSRRW, InstructionType.I, mutable.Map(UOPs.newElement("CSRRW") -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM),fuType = FU_TYPE.CSR)))
      addDecoding(Opcodes.CSRRS, InstructionType.I, mutable.Map(UOPs.newElement("CSRRS") -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM),fuType = FU_TYPE.CSR)))
      addDecoding(Opcodes.CSRRC, InstructionType.I, mutable.Map(UOPs.newElement("CSRRC") -> uop_handle(rs2 = rg_handle(source = RegisterSource.IMM),fuType = FU_TYPE.CSR)))
      addDecoding(Opcodes.CSRRWI, InstructionType.CSRI, mutable.Map(UOPs.newElement("CSRRWI") -> uop_handle(rs1 = rg_handle(is_used = false), rs2 = rg_handle(source = RegisterSource.IMM),fuType = FU_TYPE.CSR)))
      addDecoding(Opcodes.CSRRSI, InstructionType.CSRI, mutable.Map(UOPs.newElement("CSRRSI") -> uop_handle(rs1 = rg_handle(is_used = false), rs2 = rg_handle(source = RegisterSource.IMM),fuType = FU_TYPE.CSR)))
      addDecoding(Opcodes.CSRRCI, InstructionType.CSRI, mutable.Map(UOPs.newElement("CSRRCI") -> uop_handle(rs1 = rg_handle(is_used = false), rs2 = rg_handle(source = RegisterSource.IMM),fuType = FU_TYPE.CSR)))
    }
  }
  ...
  switch(ir) {
    for ((key, action) <- decodings) {
      is(key) {
        ...
        if(!(action.values.head.cycles == 1)) io.uop.cycles := U(action.values.head.cycles)
        if(!(action.values.head.fuType == FU_TYPE.ALU)) io.uop.fu := action.values.head.fuType
