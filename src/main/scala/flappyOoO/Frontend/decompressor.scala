...
object RvcDecompressor{
  def apply(rvf: Boolean, rvd: Boolean, xlen: Int, i: Bits): DecompressedInstruction = {
    val rvcDecompressor = new RvcDecompressor(rvf, rvd, xlen)
    rvcDecompressor.i := i
    rvcDecompressor.decompInstr
  }
}
class RvcDecompressor(rvf: Boolean, rvd: Boolean, xlen: Int) extends Component {
  val i = in(Bits(xlen bits))
  val decompInstr = out(DecompressedInstruction())
  ...
  switch(i(1 downto 0) ## i(15 downto 13)) {
    ...
    if (rvd) is(1) {
      ret.inst := ldImm ## rch ## B"011" ## rcl ## B"0000111"
    } // C.FLD
    is(2) {
      ret.inst := lwImm ## rch ## B"010" ## rcl ## B"0000011"
    } //C.LW -> lw rd', offset[6:2](rs1')
    if (xlen == 32 && rvf) is(3) {
      ret.inst := lwImm ## rch ## B"010" ## rcl ## B"0000111"
    } // C.FLW
    if (xlen == 64) is(3) {
      ret.inst := ldImm ## rch ## B"011" ## rcl ## B"0000011"
    } // C.LD
    is(5) {
      ret.inst := sdImm(11 downto 5) ## rcl ## rch ## B"011" ## sdImm(4 downto 0) ## B"0100111"
    } // C.FSD
    is(6) {
      ret.inst := swImm(11 downto 5) ## rcl ## rch ## B"010" ## swImm(4 downto 0) ## B"0100011"
    } //C.SW -> sw rs2',offset[6:2](rs1')
    if (xlen == 32 && rvf) is(7) {
      ret.inst := swImm(11 downto 5) ## rcl ## rch ## B"010" ## swImm(4 downto 0) ## B"0100111"
    } // C.FSW
    if (xlen == 64) is(7) {
      ret.inst := sdImm(11 downto 5) ## rcl ## rch ## B"011" ## sdImm(4 downto 0) ## B"0100011"
    } // C.SD
    is(8) {
      ret.inst := addImm ## i(11 downto 7) ## B"000" ## i(11 downto 7) ## B"0010011"
    } //C.ADDI -> addi rd, rd, nzimm[5:0].
    if (xlen == 32) is(9) {
      ret.inst := jalImm(20) ## jalImm(10 downto 1) ## jalImm(11) ## jalImm(19 downto 12) ## x1 ## B"1101111"
    } //C.JAL -> jalr x1, rs1, 0.
    if (xlen == 64) is(9) {
      ret.inst := addImm ## i(11 downto 7) ## B"000" ## i(11 downto 7) ## B"0011011"
    } //C.JAL -> jalr x1, rs1, 0.
    is(10) {
      ret.inst := lImm ## B"00000" ## B"000" ## i(11 downto 7) ## B"0010011"
    ...
    is(13) {
      ret.inst := jImm(20) ## jImm(10 downto 1) ## jImm(11) ## jImm(19 downto 12) ## x0 ## B"1101111"
    }
    is(14) {
      ret.inst := bImm(12) ## bImm(10 downto 5) ## x0 ## rch ## B"000" ## bImm(4 downto 1) ## bImm(11) ## B"1100011"
    }
    is(15) {
      ret.inst := bImm(12) ## bImm(10 downto 5) ## x0 ## rch ## B"001" ## bImm(4 downto 1) ## bImm(11) ## B"1100011"
    }
    is(16) {
      ret.inst := B"000000" ## i(12) ## i(6 downto 2) ## i(11 downto 7) ## B"001" ## i(11 downto 7) ## B"0010011"
    }
    if (rvd) is(17) {
      ret.inst := ldspImm ## x2 ## B"011" ## i(11 downto 7) ## B"0000111"
    } // C.FLDSP
    is(18) {
      ret.inst := lwspImm ## x2 ## B"010" ## i(11 downto 7) ## B"0000011"
    }
    if (xlen == 32 && rvf) is(19) {
      ret.inst := lwspImm ## x2 ## B"010" ## i(11 downto 7) ## B"0000111"
    } // C.FLWSP
    if (xlen == 64) is(19) {
      ret.inst := ldspImm ## x2 ## B"011" ## i(11 downto 7) ## B"0000011"
    } // C.LDSP
    is(20) {
      val add = B"000_0000" ## i(6 downto 2) ## (i(12) ? i(11 downto 7).asBits | x0) ## B"000" ## i(11 downto 7) ## B"0110011" //add => add rd, rd, rs2  mv => add rd, x0, rs2
    ...
    if (rvd) is(21) {
      ret.inst := sdspImm(11 downto 5) ## i(6 downto 2) ## x2 ## B"011" ## sdspImm(4 downto 0) ## B"0100111"
    } // C.FSDSP
    is(22) {
      ret.inst := swspImm(11 downto 5) ## i(6 downto 2) ## x2 ## B"010" ## swspImm(4 downto 0) ## B"0100011"
    }
    if (xlen == 32 && rvf) is(23) {
      ret.inst := swspImm(11 downto 5) ## i(6 downto 2) ## x2 ## B"010" ## swspImm(4 downto 0) ## B"0100111"
    } // C.FSwSP
    if (xlen == 64) is(23) {
