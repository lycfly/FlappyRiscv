...
import protus.backbone.{Config, Csr, CsrService, DecoderService, InstructionType, IssueService, Opcodes, Pipeline, PipelineData, Plugin, Stage, TrapCause, TrapService, Utils}
...
class MachineMode(ecallStage: Stage, addMtvec: Boolean = true, addMepc: Boolean = true)
  ...
  override def setup(): Unit = {
    ...
    pipeline.service[DecoderService].configure { config =>
      ...
      config.addDecoding(Opcodes.ECALL, I, Map(Data.ECALL -> True))
