...
import protus.backbone.InstructionType.I
import protus.backbone.JumpType.{Trap, TrapReturn}
import protus.backbone.{Config, CsrIo, CsrService, DecoderService, IssueService, JumpService, Opcodes, Pipeline, PipelineData, Plugin, Stage, StaticPipeline, TrapCause, TrapService}
...
class TrapHandler(trapStage: Stage)(implicit config: Config)
  ...
  override def setup(): Unit = {
    ...
    pipeline.service[DecoderService].configure { decoder =>
      ...
      decoder.addDecoding(Opcodes.MRET, I, Map(Data.MRET -> True))
  ...
  override def build(): Unit = {
    ...
    val trapArea = trapStage plug new Area {
      ...
      when(arbitration.isValid && value(Data.HAS_TRAPPED)) {
        ...
        jumpService.jump(trapStage, vecBase, Trap, checkAlignment = false)
      ...
      when(arbitration.isValid && value(Data.MRET)) {
        ...
        jumpService.jump(trapStage, mepc.read(), TrapReturn, checkAlignment = false)
      }
    }
