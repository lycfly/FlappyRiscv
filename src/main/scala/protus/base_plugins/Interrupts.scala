...
import protus.backbone.TrapCause.{MachineExternalInterrupt, MachineTimerInterrupt}
import protus.backbone.{Config, Csr, CsrIo, CsrService, InterruptService, IrqIo, Pipeline, Plugin, Stage, TrapService}
...
class Interrupts(interruptStage: Stage) extends Plugin[Pipeline] with InterruptService {
  ...
  override def build(): Unit = {
    val interruptArea = interruptStage plug new Area {
      ...
      when(gie) {
        when(mtie && mtip) {
          trapHandler.trap(interruptStage, MachineTimerInterrupt)
        }

        when(meie && meip) {
          trapHandler.trap(interruptStage, MachineExternalInterrupt)
        }
      }
    }
