...
import protus.backbone.JumpType.Normal
import protus.backbone.{BranchTargetPredictorService, DecoderService, JumpService, Pipeline, PipelineData, Plugin, Stage}
...
//    object PREDICTED_PC_VALID extends PipelineData(Bool())
//    object PREDICTED_PC_PAYLOAD extends PipelineData(UInt(config.xlen bits))

    object PREDICTED_JUMP extends PipelineData(Bool())
  }

//  override def getPredictPcPayload: PipelineData[UInt] = Data.PREDICTED_PC_PAYLOAD
//
//  override def getPredictPcValid: PipelineData[Bool] = Data.PREDICTED_PC_VALID
...
//        fetchStage.output(Data.PREDICTED_PC_VALID) := True
      } otherwise {
        fetchStage.output(Data.PREDICTED_PC) := fetchStage.value(pipeline.data.NEXT_PC)
//        fetchStage.output(Data.PREDICTED_PC_VALID) := False
      }
//      fetchStage.output(Data.PREDICTED_PC_PAYLOAD) :=  predictIo.predictedAddress.payload
