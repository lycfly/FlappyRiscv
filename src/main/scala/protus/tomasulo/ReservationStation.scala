...
import protus.backbone.{Config, DynBundle, DynamicPipeline, PipelineData, Resettable, Stage}
...
class ReservationStation(
  ...
  private object State extends SpinalEnum {
    val RS_IDLE, RS_WAITING_FOR_ARGS, RS_EXECUTING, RS_BROADCASTING_RESULT = newElement()
  }

  private val stateNext = State()
  private val state = RegNext(stateNext).init(State.RS_IDLE)
  ...
  override def onCdbMessage(cdbMessage: CdbMessage): Unit = {
    ...
    when(state === State.RS_WAITING_FOR_ARGS) {
    ...
    when(state === State.RS_WAITING_FOR_ARGS || stateNext === State.RS_WAITING_FOR_ARGS) {
      val r1w = Bool()
  ...
  def build(): Unit = {
    ...
    exeStage.arbitration.isStalled := state === State.RS_WAITING_FOR_ARGS
    exeStage.arbitration.isValid :=
      (state === State.RS_WAITING_FOR_ARGS) || (state === State.RS_EXECUTING)

    isAvailable := False

    when(state === State.RS_IDLE) {
      isAvailable := !activeFlush
    }
    ...
    when(state === State.RS_EXECUTING && exeStage.arbitration.isDone && !activeFlush) {
      ...
      when((cdbStream.ready || !cdbStream.valid) && dispatchStream.ready) {
        ...
        stateNext := State.RS_BROADCASTING_RESULT
      }
    }

    // if the result is on the buses and it has been acknowledged, make the RS
    // available again
    when(state === State.RS_BROADCASTING_RESULT && !activeFlush) {
    ...
    def dependencySetup(
      ...
      when(rsData.valid) {
        ...
        when(
          ...
          stateNext := State.RS_WAITING_FOR_ARGS
        }
      }
    }
